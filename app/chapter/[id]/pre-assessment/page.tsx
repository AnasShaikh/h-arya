...
function normalizeQuestions(rawQuestions: unknown, sourcePrefix: string): Question[] {
...
function normalizeQuestion(raw: RawQuestion, sourcePrefix: string, index: number): Question | null {
  ...
  const difficulty = VALID_DIFFICULTY.has((raw.difficulty ?? '').toString())
...
export default function PreAssessment() {
  ...
  const fetchAssessment = async () => {
    try {
      ...
      const primaryQuestions = normalizeQuestions(data.preAssessment, 'pre');
      if (primaryQuestions.length > 0) {
        setQuestions(primaryQuestions);
        return;
      }

      const fallbackQuestions = normalizeQuestions(data.test, 'test-fallback');
      if (fallbackQuestions.length > 0) {
        setQuestions(fallbackQuestions.slice(0, 5));
        setFallbackMessage(
          'Pre-assessment questions were unavailable for this chapter. We loaded a quick warm-up from chapter test questions instead.'
        );
        return;
      }

      setLoadError(
        'We could not prepare a pre-assessment for this chapter yet. You can go to the explanation directly and continue learning.'
      );
    } catch (error) {
      console.error('Error fetching assessment:', error);
      setLoadError('Could not load this pre-assessment right now. Please retry or continue to chapter explanation.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-violet-50 to-indigo-50 flex items-center justify-center p-4">
        <div className="max-w-xl w-full bg-white rounded-3xl shadow-lg p-8 border border-violet-100">
          <h1 className="text-2xl font-bold text-gray-900 mb-3">Pre-Assessment Unavailable</h1>
          <p className="text-gray-700 mb-6">
            {loadError || 'We could not prepare valid pre-assessment questions for this chapter yet.'}
          </p>
      ...
      <div className="max-w-3xl mx-auto py-4 md:py-8">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6 md:p-8 mb-6 border border-violet-100">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 leading-tight">Pre-Assessment Quiz</h1>
              <p className="text-sm text-gray-500 mt-1">Let&apos;s check what you already know!</p>
            </div>
            <span className="inline-flex items-center px-4 py-2 bg-violet-50 text-violet-700 text-sm rounded-full font-bold border border-violet-100">
              Stage 1 of 5
            </span>
          </div>
