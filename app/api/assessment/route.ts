...
const fallbackQuestions: Question[] = [
  { id: 'q1', question: 'How is sound produced?',
    options: ['By the vibration of an object', 'By light waves', 'By heat energy', 'By magnetic fields'],
    correctAnswer: 'A', difficulty: 'easy' },
  { id: 'q2', question: 'What is the unit used to measure the frequency of sound?',
    options: ['Decibel (dB)', 'Hertz (Hz)', 'Meter (m)', 'Second (s)'],
    correctAnswer: 'B', difficulty: 'easy' },
  { id: 'q3', question: 'What is the frequency range of sound audible to human beings?',
    options: ['0 Hz to 20 Hz', '20 Hz to 20,000 Hz', '20,000 Hz to 50,000 Hz', '10 Hz to 10,000 Hz'],
    correctAnswer: 'B', difficulty: 'medium' },
  { id: 'q4', question: 'Sound with frequency less than 20 Hz is called:',
    options: ['Ultrasonic sound', 'Audible sound', 'Infrasonic sound', 'Supersonic sound'],
    correctAnswer: 'C', difficulty: 'medium' },
  { id: 'q5', question: 'Which of these is a use of ultrasonic sound?',
    options: ['Listening to music', 'SONAR - to locate objects underwater', 'Making phone calls', 'Watching television'],
...
export async function GET() {
  try {
    try {
      ...
      if (jsonMatch) {
        return NextResponse.json({ questions: JSON.parse(jsonMatch[0]) as Question[] });
      }
    } catch {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!studentId || !answers) {
      return NextResponse.json({ error: 'Student ID and answers are required' }, { status: 400 });
    }
    ...
    questions.forEach(q => {
      ...
      answersArray.push({ questionId: q.id, userAnswer, isCorrect });
    });
    ...
    await createAssessment(studentId, 'Science', 'Sound', questions, answersArray, score, questions.length);
